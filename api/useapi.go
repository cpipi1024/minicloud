...
func GetUserByID(c *gin.Context) {
	...
	if err != nil {
		Fail(c, bootstrap.CodeRequestParmaFailed, "id类型错误")
		c.Abort()
		return
	}
...
func QueryUsers(c *gin.Context) {
	...
	if err != nil {
		Fail(c, bootstrap.CodeRequestParmaFailed, "查询表单错误")
		c.Abort()
		return
	}
...
func UserLogin(c *gin.Context) {
	...
	if err != nil {
		Fail(c, bootstrap.CodeRequestParmaFailed, "登录表单错误")
		c.Abort()
		return
	}
	...
	if err != nil {
		...
		if errors.Is(e, bootstrap.ErrMySQLOptFailed) {
			bootstrap.ApiError("[user login failed]", c.Request.Method, c.Request.URL.Path, e.Code, e.Error())
		}

		if errors.Is(e, bootstrap.ErrLoginFailed) {
			bootstrap.ApiError("[user login failed]", c.Request.Method, c.Request.URL.Path, e.Code, e.Error())
		}

		if errors.Is(e, bootstrap.ErrJWTAuthFailed) {
			bootstrap.ApiError("[user login failed]", c.Request.Method, c.Request.URL.Path, e.Code, e.Error())
		}
...
func UserRegister(c *gin.Context) {
	...
	if err != nil {
		Fail(c, bootstrap.CodeRequestParmaFailed, "注册表单错误")
		c.Abort()
		return
	}
	...
	if err != nil {
		...
		if errors.Is(e, bootstrap.ErrRegisterFailed) {
